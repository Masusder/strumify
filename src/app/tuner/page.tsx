        ...
        <div className='m-auto bg-customBg'>
            <div className='flex flex-col-reverse justify-center max-w-screen-2xl m-auto items-center lg:flex-row gap-8'>
                    ...
                    <div className='w-[300px] h-[407.22px] relative'>
                        <div className={styles.tuningGrid + " absolute"}>
                            {guitarData.tuningNotation.map((note: string, index: number) => {
                                return <button key={index} onClick={() => changeTuningMode(index)} style={{border: cents !== 0 && targetedNoteIndex === index ? `1px solid ${updateElementColor()}` : ''}} className={`${styles.tuningNote} ${styles[`tuningNote${index + 1}`]} ${targetedNoteIndex === index ? styles.noteTargetedGrid : ''}`}>{note}</button>;
                            })
                            }
                        </div>
                ...
                <div className='flex items-center flex-col-reverse lg:flex-col'>
                    ...
                    <Button className={isPitchDetectionRunning ? 'bg-lime-600 hover:bg-lime-500 mt-3' : 'mt-3'} onClick={togglePitchDetection}>{isPitchDetectionRunning ? 'STOP' : 'Click to start tuning'}</Button>
                </div>
            </div>
